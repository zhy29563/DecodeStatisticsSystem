...
namespace DecodeStatisticsSystem
{
    public class IniHelper
    {
        ...
        /// <param name="filePath">INI文件的完整路径和文件名</param>
        /// <returns>
        ///     复制到缓冲区的字节数量，其中不包括那些NULL中止字符。
        ///     若缓冲区size不够大，不能容下全部信息，就返回nSize-1
        ///     若section或key为NULL，则返回nSize-2
        /// </returns>
        [DllImport("kernel32")]
        ...
        {
            var size = WritePrivateProfileString(section, keyName, keyValue, FullName);
            if (size == 0) throw new Exception($"Writing data to ini file is fail{Environment.NewLine}{FullName}");
        }
        ...
        {
            WriteIni(section, keyName, keyValue.ToString(CultureInfo.CurrentCulture));
        }
        ...
        public string ReadIni(string section, string keyName, string defaultValue)
        {
            ...
            long size = GetPrivateProfileString(section, keyName, defaultValue, builder, ReadBufferSize, FullName);
            if (size == ReadBufferSize - 1)
                throw new Exception("The read buffer is small, please change the buffer size");
            if (size == ReadBufferSize - 2) throw new Exception("The section or key is not found, please check it");
        ...
        {
            int ret;
            var str = ReadIni(section, keyName, defaultValue.ToString());
        ...
        {
            double ret;
            var str = ReadIni(section, keyName, defaultValue.ToString(CultureInfo.CurrentCulture));
